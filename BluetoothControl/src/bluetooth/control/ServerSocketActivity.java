...
public class ServerSocketActivity extends ListActivity
{
	...
	/* Thread-Listen */
	private Thread _serverWorker = new Thread() { public void run() { listen(); }; };
		...
		new Thread() {
			public void run() {
				...
				if (_serverSocket != null) {
					try { /* close sever */ _serverSocket.close(); } 
					catch (IOException e) { Log.e("EF-BTBee", "", e); }
					_serverSocket = null;
				}
			};
		}.start();
	...
	public void onButtonClicked(View view) { shutdownServer(); }
		...
		try {
			...
			_handler.post(new Runnable() {
				public void run() {
					...
					ArrayAdapter<String> adapter = new ArrayAdapter<String>( ServerSocketActivity.this,
				...
				for (; (read = inputStream.read(bytes)) > -1;) {
					...
					_handler.post(new Runnable() {
						public void run() {
							...
							for (int i = 0; i < count; ++i) { if (i > 0) { b.append(' '); }
